CREATE OR REPLACE PACKAGE BODY ALG_IMPCLT is
  ...
  begin
   ...
   and p.numord (+) = substr(pntlvr, instr(pntlvr, '-', -1) + 1) ;
    ...
    update MIGR_Clt set IdtCpr  = (select Cpr.IdtCpr
  ...
  Procedure Renumord is
    ...
    cursor cCur is
      select nvl(migr_Clt.IdtDcpGgr, migr_prm.idtdcpggr) idtdcpggr
        from MIGR_CLT, migr_prm where migr_clt.idtpntlvr is null
  ...
  begin
    ...
    update MIGR_CLT
       set SwMgr = 11
     where numcpr in(select numcpr from impclt.migr_clt
group by numcpr
having count(*)>1)
       and EXISTS (select IdtCtrl
              from PRMCONTROL
...
having count(*)>1)
    ...
    update MIGR_CLT
       set SwMgr = 12
     where numcpr in(select numcpr from impclt
intersect
select numcpr from X7.CPR, X7.CPRPNTCPG
  ...
  BEGIN
    ...
    loop
      ...
      for i1 in 1 .. tcur_MIGR_CLT.count loop
        ...
        if tcur_MIGR_CLT(i1)
         .NumCpr is not null and tcur_MIGR_CLT(i1).idttypcpr is not null then
          ---------------------------------------------------------------------
          ...
          if tcur_MIGR_CLT(i1).NumCpr is not null then
            ...
            begin
						 Select Cpr.IdtCpr into tcur_MIGR_CLT(i1).IdtCpr
             from X7.CPR, X7.CprPntCpg
            ...
            if tcur_MIGR_CLT(i1).IdtCpr is null then
               MIGRATION.INSCPR(tcur_MIGR_CLT(i1));
					  ELSE 
							dbms_output.put_line('Compteur: '||tcur_MIGR_CLT(i1).NumCpr || ' existe:'||tcur_MIGR_CLT(i1).IdtCpr);
							tcur_MIGR_CLT(i1).NumCpr := tcur_MIGR_CLT(i1).NumCpr ||'-IMP';
        ...
        if i1 = tcur_MIGR_CLT.count then
          ...
          exception
            when others then
              dbms_output.put_line('Erreur : Sqc -->' || tcur_MIGR_CLT(i1)
                                   .Idtsqc || ' ' || Sqlerrm);
              raise;
          end;
        end if;
