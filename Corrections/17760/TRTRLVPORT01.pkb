...
-- Maj     : 21/03/2006
-- Par     : SJ
-- Contenu : fiche 4205 - Adaptation des proc�dures Oracle pour bfex: utilise les champs param�trables du pntcpg
...
--mise � jour de la table pntcpg � partir de la rel�ve portable, etc
-----------------------------------------------------------------------------
-----------------------------------------------------------------------------
-- Maj     : 14/06/2006
-- Par     : SJ
-- Contenu : Regroupement par tourn�es pour plusieurs lot - pas de fiche tracker  -
--fiche mantis 4459
-----------------------------------------------------------------------------
-----------------------------------------------------------------------------
-- Maj     : 09/08/2006
...
-- Maj     : 30/08/2006
-- Par     : SJ
-- Contenu : r�cupere prmgnr.sprcdf � l'import car il n'est pas utilis� dans rlvport.exe (pour ne pas grandir la taille de fichier � l'export)
...
-- Maj     : 22/11/2006
-- Par     : FP
-- Contenu : Modification du package Rlvportable, proc�dure DCHINDRLVPORT
-- pour permettre l'utilisation de la colonne BCHINC
-----------------------------------------------------------------------------
-----------------------------------------------------------------------------
-- Maj     : 25/10/2006
-- Par     : SJ
-- rendre la cr�ation des tourn�es de rel�ve possible si le nombre des compteurs dans la tourn�e d�poasse 353
-- fiche 4584.
-- VERSION 10201
-----------------------------------------------------------------------------
-----------------------------------------------------------------------------
-- Maj     : 31/10/2006
-- Par     : SJ
-- utiliser une table temporaire pour cr�er l'hst des indexes
-- fiche 4584.
-- VERSION 10201
-----------------------------------------------------------------------------
-----------------------------------------------------------------------------
...
-- Maj     : 07/04/2009
-- Par     : SJ
-- Contenu : mettre idtcodorg � 4 si il est null
-----------------------------------------------------------------------------

-----------------------------------------------------------------------------
-- Maj     : 18/04/2009
-- Par     : BPZ
-- Contenu : Dans certain cas les infos compl�mentaires ne sont pas transf�r�es dans X7
-----------------------------------------------------------------------------

-----------------------------------------------------------------------------
-- Maj     : 28/08/2009
-- Par     : SJ
-- Contenu : Fiche 7700 - Erreur de mise � jours de taches au cas o?� lots non regroup�
-----------------------------------------------------------------------------

-----------------------------------------------------------------------------
-- Maj     : 30/11/2009
-- Par     : FP
-- Contenu : utilisation de DBMS_XMLGEN � la place de system.xmlgen
-----------------------------------------------------------------------------
-----------------------------------------------------------------------------
-- Maj     : 27/01/2010
-- Par     : SJ
-- Contenu : modifier le nom des fichiers de parametrage et agt = pr�fix 'typo_' fiche 7867
-----------------------------------------------------------------------------
-----------------------------------------------------------------------------
-- Maj     : 27/01/2010
-- Par     : FP
-- Contenu : - g�n�ration de 2 fichiers : r�sum� de la tourn�e TRNxxxx et donn�es de la tourn�e
...
--           - dans la fonction TrnHstIndExport, on enl�ve le FOR UPDATE (qui fonctionne mal en oracle 10) du cursor c_lot
-----------------------------------------------------------------------------
-----------------------------------------------------------------------------
-- Maj     : 01/06/2010
-- Par     : SJ
-- Contenu : - traitement de la fiche 8425 - enlever temporairement le controle date et heure de rel�ve (jusqu'au moment quand la syncronisation de la date et
...
-- Maj     : 11/08/2010
-- Par     : FP
-- Contenu : - modification de la requ�te pour la cr�ation du fichier de r�sum� des tourn�es de mani�re � supprimer
...
-- Par     : FP
-- Contenu : - on essaye de r�duire les temps de g�n�ration des fichiers, pour ce faire :
--                * ajout d'une fonction exp_file r�cup�r�e du recensement
--                * utilisation de cette fonction pour cr�er les fichiers xml
-----------------------------------------------------------------------------
-----------------------------------------------------------------------------
-- Maj     : 04/08/2011 - 20204.009
-- Par     : SJ
-- Contenu : - le nombre total de compteurs de la tourn�e vNbrCprTrn = TRNTYPRLV.nbrcpr - le nombre des compteurs extraits du lot aux moment de l'export
--       +   exporter tous les donn�es des compteurs qui sont dans RLV m�me sans ctr car il est possible de vouloir lire les cpr inactif (si prmgrn.pntcpginc=1 alors dans la table rlv on peut avoir cpr inactifs, �a c'est d�j� fait dans calslf01 et rlvbat01)
-- Note FP: on corrige dans cette version une erreur apparue le 09/05/2012 car j'ai oubli� d'envoy� ce package lors de la livraison de la rel�ve portable
--          voir fiche 0010111
-----------------------------------------------------------------------------
-----------------------------------------------------------------------------
-- Maj     : 03/07/2012 - 20205.010
-- Par     : FP
-- Contenu : - fiche 10071 : on utilise � pr�sent 2 directories seulement
...
-- VERSION : 30102.018
-- Par     : BPZ
-- Mantis    11735: Division de tourn�es
-----------------------------------------------------------------------------
-- Maj     : 12/07/2014
-- VERSION : 30200.026
-- Par     : FBE
-- Contenu : - 11874: Adresses longues tronqu�es sur le PDA
--           - 11875: Probl�mes de dates lorsque le d�passe minuit
--           - 11877: Gestion de la division de tourn�es
--           - 11878: Possibilit� d'annuler la g�n�ration de fichiers rel�ve en �tat 4
--           - 11879: Modification du syst�me d'historisation des fichiers de la rel�ve portable
-----------------------------------------------------------------------------
-----------------------------------------------------------------------------
-- Maj     : 05/03/2015
-- Par     : SJ
...
--  - Le point d�entr�e des taches de g�n�ration est STT=�L�, �hors que nous sur le carnet de rel�ve nous mettons les STT=�T� s�il y au moins nue TRN portable sur le lot.
--  - Sur le package TRTRLVPORT01 on met � jour IdtEtTrn � 5 avant de g�n�rer le fichier XML (voir lign 441)...
--  --SJ : gestion des taches comme avant et meilleurs gestion des erreurs: si pb � la g�n�ration on ne mets pas � jour l'�tat de la tourn�e trn
...
-- Par     : SJ
-- VERSION : 30204.028
-- Contenu : - traitement de la fiche 12466: reprise des modifications de FNE sauf la maj de la tache � null si la tache est � 'T'
-- Ci-dessous le fonctionnement des changement des �tat de la tache TRT2003:
--signification des �tats L, T, C, F pour idttyptche=2003 qui a �t� connstruit pour la Roumanie et repris en version produit:
--
-- 1. le carnet de rel�ve est imprim� par un �tat brio lanc� avant l'appel de la proc�dure d'exp de la rel�ve portable

--Ce carnet de rel�ve � la fin du traitement de la tourn�e met la tache en F si toutes les tourn�es sont en portable ou si on imprime que le carnet de rel�ve m�me pour les tourn�es en portable. Le traitement d'export de la rel�ve portable ne doit rien faire dans ce cas. Lorsque le carnet de rel�ve commence le traitement la tache est mise � C (en cours).
--A la fin du traitement le carnet de rel�ve met l'�tat de la t�che � F ou � T comme suite:
...
--2.le traitement de l'export de la rel�ve portable prend la tache qui a stt=null ou T et la passe en L (lancement de l'exp portable) qui est �quivalent de C "en cours" pour le carnet de rel�ve ou autre type de t�che).
--Si le traitement est intrerrompu , le traitement de la rel�ve portable doit reprendre les taches qui sont en �tat null ou L (commenc� mais pas fini par l'exp portable) et en �tat T ( trait� par le carnet de rel�ve des tourn�es pas en portable et disponible pour l'export des autres tourn�es)
...
-- Par     : FNE
-- VERSION : 30204.030
-- Contenu : 17760: Gestion des coefficients des Compteurs (TYPCPR.CFF) lors de la d�charge
-----------------------------------------------------------------------------

--FUNCTION VersionPkg RETURN VersionPackage
----NUMBER
--IS
--pour le cas o� on veux liverer en double mais je pense que il ne faut pas faire �a car risque d'err et necessaire d'autre modif sur traccage
...
--NB : pPath n'est plus utilis�, on utilise les directories
--r�pertoire de destination sur le serveur de base de donn�es (il faut ULT_FILE_DIR=* ou alors ajouter les droits
--en �criture sur le r�pertoire)
  ...
  cursor cLot is
    select LOTFAC.IdtLotFac,
       ...
       where LOTFAC.IdtLotFac = TCHE.Prm2
        ...
        and (TCHE.Stt is null or TCHE.Stt = 'T' or TCHE.Stt = 'L') -- SJ 05/03/2015 traitement de la fiche 12407 : pour des raison de comptaibilit� Alger lance les carnet de rel�ve et � la fin si il reset au moins une tourn�e en "portable" la tcahe est mise � T
...
--           and  Sqc = rTRN.Sqc;
    ...
    --SJ 05/03/2015, traitement de la fiche 12407 -�viter la cr�ation d'un fichier vide : � bien tester cela car je ne suis pas trop d'acc avec le code ci-dessus

      if vNbr != 0 then
           -- FNE: 12466 gestion de l'export
 -- �a emp�che la g�n�ration des trn vides
            begin
                erreur:=false;
      -----------------------------------------------------------
...
--'
      ...
      Insert into sys_hst_msg (NOM_UTILISATEUR,NOM_TRAITEMENT,DAT_COURANTE,MSG_CODE,MSG_LIBELLE,NUM_PROGRESSION,NOM_LOCALISATION,CPT,NOM_REQUETE )
                      values    (user, 'TRTRLVPORT.export',sysdate, 'MSG_INF2002', '', 0,'TRTRLVPORT.export',null, 'pas de donn�es pour la tournn�e ' || rTRN.IdtAgt || '-' || vIdtLotFacRgr || '-' || rTRN.IdtTrnTyp || '_' || rTRN.Sqc || '-' || rTRN.DatPrvRlv   );

      DBMS_OUTPUT.PUT_LINE('pas de donn�es pour la tournn�e ' || rTRN.IdtAgt || '-' || vIdtLotFacRgr || '-' || rTRN.IdtTrnTyp || '_' || rTRN.Sqc || '-' || rTRN.DatPrvRlv);

     end if;
--'
  --SJ 05/03/2015 - traitement de la fiche 12407 : v�rif que l'export est bien fait avant de maj de l'�tat de la tourn�e
      -- Mise a jour de l'etat de la tournee
if not erreur then
forall i in 1..tIdtLotFac.Count
        update TRN
...
--      DBMS_OUTPUT.Put_Line(vCodMsg || '-' || vMsg);
  ...
  for i in 1..tXML.Count loop
    ...
    BEGIN
      ...
      -- FNE: 12466 ajout�, gestion des valeurs Varchar2: null
      ------------------------------------------
      vChamp := 'IdtLotFac';
...
--  EXCEPTION
--    when others then
--      null;  --FP 10/07/2012 le fichier historique n'est plus cens� exister depuis la version 20500 et donc on ne fait rien si exception
--  END;
