...
ECHO . >> %LOG_DIR%\%LOG_FILE%
ECHO ----------------------------------- >> %LOG_DIR%\%LOG_FILE%
ECHO Importation fichier Algerie Poste   >> %LOG_DIR%\%LOG_FILE%
ECHO ----------------------------------- >> %LOG_DIR%\%LOG_FILE%
ECHO . >> %LOG_DIR%\%LOG_FILE%
ECHO .
ECHO -----------------------------------
...
for %%a in ("%IMPORT_DIR%\AP_IN_*.txt") do call "%SQRW_DIR%\%SQRW_BAT%" "%SQT_DIR%\AlgEap02.sqt" %LG%/%PWD%@%ORACLE_SID% -%COMMBOX% -%IMP_TYP% %AGENT% %MOD_DCL% "%SPL_DIR%" "%LOG_DIR%\%LOG_FILE%" "%IMPORT_DIR%" %%a -KEEP -ZIV -O%SQR_LOG_FILE%
