CREATE OR REPLACE PACKAGE BODY X7."ALG_FCTCTL" IS
  -- ==========================================================================
  ...
  PROCEDURE CtlPntCpg(pIdtPntLvr IN PNTLVR.IdtPntLvr%TYPE,
    ...
    VA2 PNTCPG.A2%TYPE;
    VA3 PNTCPG.A3%TYPE;
    Vpntlvr PNTCPG.idtpntlvr%TYPE;
    Vpntcpg PNTCPG.idtpntcpg%TYPE;
    VSqcpntcpg PNTLVR.Sqcpntcpg%TYPE;
    vnbrcpg number;
  ...
  BEGIN
   ...
   --FDR-0015-test yahia
    SELECT  PNTCPG.A1, PNTCPG.A2, PNTCPG.A3 ,pntlvr.idtpntlvr,idtpntcpg, sqcpntcpg 
    into VA1, VA2, VA3 ,Vpntlvr ,Vpntcpg, vSqcPntCpg
    FROM pntcpg , pntlvr
     ...
     and PntLvr.IdtPntLvr = pIdtPntLvr
     and PNTCPG.IdtPntCpg = pIdtPntCpg;   
        
     if ( nvl(VA1,'~') <> nvl(pdfA1,'~') or 
          nvl(VA2,'~') <> nvl(pdfA2,'~') or 
          nvl(VA3,'~') <> nvl(pdfA3,'~')) and PiDTpNTcPG = vSqcPntCpg and vSqcpntcpg!=1
       then
     
     sTypMsg    := '0';
     sRetFctCtl := 'Vous n''êtes pas autorisé à modifier latitude, longitude ou Etg de distrib !!!'||' '||nvl(VA1,'~')||
                   '  '||nvl(VA2,'~')||
                   '  '||nvl(VA3,'~')||
                   '  '||substr(pIdtDcpGgrNumOrd, 1, instr(pIdtDcpGgrNumOrd, '-', -1) - 1)||
                   '  '||substr(pIdtDcpGgrNumOrd, instr(pIdtDcpGgrNumOrd, '-', -1) + 1)||
                   '  '||nvl(pdfA1,'~')||' '||nvl(pdfA2,'~')||' '||nvl(pdfA3,'~')||
                   ' idtpntlvr :'||Vpntlvr||
                   ' NOUVEAU idtpntlvr :'||pidtpntlvr||
                   ' idtpntcpg :'||pIdtPntCpg||
                   ' Sqcpntcpg :'||vSqcPntCpg;
     end if;
     exception
    when NO_DATA_FOUND then
      if (pdfA1 is not null or pdfA2 is not null or pdfA3 is not null ) and vPntLvr is not null then
      sTypMsg    := '0';
      sRetFctCtl := 'Erreur vous ne pouvez pas saisir les informations de l''un des champs suivants latitude, longitude
...
,'CPRFD','DEPDEL'
     ...
     then
     sTypMsg    := '0';
     sRetFctCtl := 'Vous n''avez pas le droit de choisir ce motif pour abandonner ce type de PDA bla bla  rodbalek said!!';
    end if;
    return;
  end CTLAFFCNTPRC;
...
BEGIN
 ...
 IF 1=1 then
        sTypMsg    := '0';
        sRetFctCtl := 'Erreur !!! '; --||pIdtTrn||'  '||pDatRlv;
end if;
--RETURN;
 ...
 Procedure Ctldatr(ptrn in VARCHAR2,
  ...
  begin
   if ptrn is not null then
  sTypMsg    := '0';
  sRetFctCtl := 'Date est:' || 'correcte';
  RETURN;
       end if;
  end Ctldatr ;
  Procedure CtldatRlvMss( pColDatRlv in VARCHAR2,
                          ...
                          sTypMsg    OUT VARCHAR2,
                          sRetFctCtl OUT VARCHAR2) is
  vDatRlv Varchar2(24);--Rlv.DatRlv%type;
  vNumCpr Cpr.NumCpr%type;
  begin
  BEGIN
  select DatRlv, NumCpr into vDatRlv, vNumCpr
    from (WITH DATA AS (SELECT pColDatRlv DatRlv,
                              pColNumCpr numcpr
                          FROM dual)
          SELECT substr(trim(regexp_substr(DatRlv, '[^|]+', 1, LEVEL)), 1, 10) DatRlv,
                  trim(regexp_substr(NumCpr, '[^|]+', 1, LEVEL)) numCpr,
                  level
            FROM DATA
          CONNECT BY instr(DatRlv, '|', 1, LEVEL - 1) > 0)
          --where str like '%5%' or str like'%3%'
            where to_date(DatRlv, 'YYYY/MM/DD') > sysdate
              and rownum = 1;
    exception
      when no_data_found then -- on ne fait rien si on ne trouve pas de problème de dates
        null;
    END;
    if vDatRlv is not null then
      sTypMsg    := '0';
      sRetFctCtl := 'la Date de relève: ' || to_char(to_date(vDatRlv, 'YYYY/MM/DD'), 'dd/mm/yyyy') || ' est erronée pour le compteur: '|| vNumCpr;--pColDatRlv ;
      -- sRetFctCtl := 'la Date de relève:' || pColDatRlv || 'est erronée pour le compteur: '|| pColNumCpr;--pColDatRlv ;
      RETURN;
    end if;
  end CtldatRlvMss ;
